...
exports.getApplicationIdAsync = exports.setPackageInAndroidManifest = exports.setPackageInBuildGradle = exports.renamePackageOnDiskForType = exports.renamePackageOnDisk = exports.getPackage = exports.withPackageRefactor = exports.withPackageGradle = exports.withPackageManifest = void 0;
...
const debug = debug_1.default('config-plugins:android:package');
...
function getCurrentPackageName(projectRoot, packageRoot) {
    const mainApplication = Paths_1.getProjectFilePath(projectRoot, 'MainApplication');
...
function getCurrentPackageForProjectFile(projectRoot, packageRoot, fileName, type) {
    const filePath = glob_1.sync(path_1.default.join(projectRoot, `android/app/src/${type}/java/**/${fileName}.@(java|kt)`))[0];
    if (!filePath) {
        return null;
    }
...
async function renamePackageOnDiskForType({ projectRoot, type, packageName, }) {
    ...
    const packageRoot = getPackageRoot(projectRoot, type);
    // Set up our paths
    if (!(await modules_1.directoryExistsAsync(packageRoot))) {
    ...
    const filesToUpdate = [...glob_1.sync('**/*', { cwd: newPackagePath, absolute: true })];
...
async function getApplicationIdAsync(projectRoot) {
    var _a;
    const buildGradlePath = Paths_1.getAppBuildGradleFilePath(projectRoot);
